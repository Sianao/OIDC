...
func MakeToken(basicinfo interface{}) string {
	v, ok := basicinfo.(models.BasicInfo)
	if ok {
		myClaim := MyClaims{
			v,
			jwt.StandardClaims{
	...
	v1, ok := basicinfo.(models.IdToken)
	if ok {
		myClaim := Claims{
			v1,
			jwt.StandardClaims{
...
func ParseIdToken(tokens string) (*models.IdToken, error) {
	if tokens == "" {
		err := errors.New("你是谁 验证信息不知道")
		return nil, err
	}
